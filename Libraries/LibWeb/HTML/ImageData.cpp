...
WebIDL::ExceptionOr<void> ImageData::serialization_steps(SerializationRecord& serialized, bool for_storage, SerializationMemory& memory)
{
    ...
    serialize_primitive_type(serialized, static_cast<u32>(m_color_space));
...
WebIDL::ExceptionOr<void> ImageData::deserialization_steps(ReadonlySpan<u32> const& serialized, size_t& position, DeserializationMemory& memory)
{
    ...
    auto deserialized_record = TRY(structured_deserialize_internal(vm, serialized, realm, memory, position));
    auto value = deserialized_record.value;
    if (value.has_value() && value.value().is_object() && is<JS::Uint8ClampedArray>(value.value().as_object())) {
        m_data = as<JS::Uint8ClampedArray>(deserialized_record.value.release_value().as_object());
    }
    position = deserialized_record.position;

    // 2. Initialize value's width attribute to serialized.[[Width]].
    auto width = deserialize_primitive_type<int>(serialized, position);

    // 3. Initialize value's height attribute to serialized.[[Height]].
    auto height = deserialize_primitive_type<int>(serialized, position);

    // 4. Initialize value's colorSpace attribute to serialized.[[ColorSpace]].
    m_color_space = static_cast<Bindings::PredefinedColorSpace>(deserialize_primitive_type<u32>(serialized, position));
